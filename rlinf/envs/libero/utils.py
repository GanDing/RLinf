...
def get_task_suite_map(benchmark_name) -> list[str]:
    """Return a list mapping ``task_id`` -> suite name for a benchmark.

    The ``task_id`` indices match those produced by ``LiberoEnv`` (i.e. the
    benchmark's task order). For ``libero_130`` this mirrors the aggregation
    order in :func:`get_benchmark_overridden` (suite order, de-duplicated by
    task name) so each aggregated task is attributed to the suite it first
    appeared in. For a single suite every task maps to that suite.

    Used by the eval runner to report per-suite success rates.
    """
    name = str(benchmark_name).lower()
    if name == "libero_130":
        suite_per_task: list[str] = []
        seen: set[str] = set()
        suites = getattr(benchmark, "libero_suites", [])
        for suite_name in suites:
            suite_map = benchmark.task_maps.get(suite_name, {})
            for task_name in suite_map:
                if task_name not in seen:
                    seen.add(task_name)
                    suite_per_task.append(suite_name)
        return suite_per_task

    bench = get_benchmark_overridden(benchmark_name)()
    return [name] * bench.get_num_tasks()
