...
class LiberoEnv(gym.Env):
    ...
    def _record_metrics(self, step_reward, terminations, infos):
        ...
        # In eval, tag each (about-to-terminate) trajectory with its absolute
        # task id so the eval runner can break success down per-task / per-suite
        # (see compute_grouped_success_metrics). task_ids still reflect the
        # current episode here because reset happens after _record_metrics.
        if getattr(self.cfg, "is_eval", False):
